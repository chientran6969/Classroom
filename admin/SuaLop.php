...
<body>
    ...
    if (isset($_POST['savechanges'])) {
        ...
        if ($tenlop == '' || $phong == '' || $mon == '') {
            ...
            //Xóa link của hình cũ trong database
            $conn = open_database();
            $sql2 = "SELECT * from lop where IdLop = ?";
    ...
    // dropdown list giáo viên
    $conn = open_database();

    $stmt1 = $conn->prepare("select * from nguoidung where Role = 1");

    if(!$stmt1->execute()){
        die("Query error:" . $stmt->error);
    }

    $resultGV = $stmt1->get_result();
                            ...
                            <div class="card shadow-lg border-0 rounded-lg mt-5">
                                ...
                                <div class="card-body">
                                    <form method="post" action="SuaLop.php" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            <label for="exampleFormControlSelect1">Giảng viên</label>
                                            <select name="GV" class="form-control" id="exampleFormControlSelect1">
                                                <?php
                                                while($listGV = $resultGV->fetch_assoc()){
                                                    if($listGV['IdNguoiDung'] == $GV){
                                                        ?>
                                                        <option selected value="<?= $listGV['IdNguoiDung']?>"><?=$listGV['Name']?></option>

                                                        <?php
                                                    }
                                                    else {
                                                        ?>
                                                        <option value="<?= $listGV['IdNguoiDung']?>"><?=$listGV['Name']?></option>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input name="fileAnh" class="form-control-file" id="fileToUpload" type="file" />
                                        </div>
                                        <div class="form-group">
                                            <?php
                                            if (!empty($error)) {
